...
class SchoolProvider extends ChangeNotifier {
  ...
  Future<void> login(
      {required BuildContext context, var email, var password, role}) async {
    ...
    SchoolManagerApi()
      ...
      if (value == 'login_error') {
        showErrorSnackBar(
            context, "Incorrect student Login Details - Restart app");
        return;
  ...
  Future<void> assessmentList() async {
    ...
    SchoolManagerApi().AssessmentTypeApi().then((value) {
      if (value.isEmpty) {
        return;
      } else {
        print('TESTING');
